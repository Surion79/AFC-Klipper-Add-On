...
class AFC_poop:
    ...
    def poop(self):
        ...
        step = 1
        if self.verbose:
            self.logger.info("AFC_Poop: {0} Move To Purge Location".format(step))
        poop_pos = self.toolhead.get_position()
        poop_pos[0] = float(self.purge_loc_xy.split(',')[0])
        poop_pos[1] = float(self.purge_loc_xy.split(',')[1])
        self.toolhead.manual_move(poop_pos, 100)
        self.toolhead.wait_moves()
        poop_pos[2] = self.purge_start
        self.toolhead.manual_move(poop_pos, 100)
        self.toolhead.wait_moves()
        step += 1
        if self.full_fan:
            if self.verbose:
                self.logger.info("AFC_Poop: {0} Set Cooling Fan to Full Speed".format(step))
        ...
        while iteration < int(self.purge_length / self.max_iteration_length):
            if self.verbose:
                self.logger.info("AFC_Poop: {0} Purge Iteration {1}".format(step, iteration))
            ...
            poop_pos = self.toolhead.get_position()
            poop_pos[2] += raise_z
            poop_pos[3] += extrude_amount
            self.toolhead.manual_move(poop_pos, speed)
            self.toolhead.wait_moves()
            iteration += 1
        step += 1
        if self.verbose:
        ...
        poop_pos = self.toolhead.get_position()
        poop_pos[2] = self.z_lift
        self.toolhead.manual_move(poop_pos, self.fast_z)
        self.toolhead.wait_moves()
        step += 1
        if self.full_fan:
            if self.verbose:
                self.logger.info("AFC_Poop: {0} Restore fan speed and feed rate".format(step))
