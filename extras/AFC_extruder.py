...
class AFCExtruder:
    def __init__(self, config):
        ...
        # Distance in mm from the toolhead sensor to the tip of the nozzle in mm, if `tool_end` is defined then distance is from this sensor
        ...
        # Useful for when only using post sensor, so this distance can be the amout to move to clear extruder gears
        ...
        # Buffer to use for extruder, this variable can be overridden per lane
        self.buffer_name = config.get('buffer', None)

        # Set to True toolhead sensors switches as filament sensors in mainsail/fluidd gui, overrides value set in AFC.cfg
        ...
        if self.tool_end is not None:
            ...
            if self.enable_sensors_in_gui:
                ...
                self.fila_avd = add_filament_switch(self.tool_end_state_filament_switch_name, self.tool_end,
                                                    self.printer)
    ...
    def handle_connect(self):
        ...
        self.AFC.gcode.register_mux_command('UPDATE_TOOLHEAD_SENSORS', "EXTRUDER", self.name,
                                            self.cmd_UPDATE_TOOLHEAD_SENSORS,
                                            desc=self.cmd_UPDATE_TOOLHEAD_SENSORS_help)
        self.AFC.gcode.register_mux_command('SAVE_EXTRUDER_VALUES', "EXTRUDER", self.name,
                                            self.cmd_SAVE_EXTRUDER_VALUES, desc=self.cmd_SAVE_EXTRUDER_VALUES_help)

    def tool_start_callback(self, state):
        self.tool_start_state = state

    def tool_end_callback(self, state):
    ...
    def cmd_SAVE_EXTRUDER_VALUES(self):
        """
    ...
    def get_status(self):
        response = {'tool_stn': self.tool_stn,
                    'tool_stn_unload': self.tool_stn_unload,
                    'tool_sensor_after_extruder': self.tool_sensor_after_extruder,
                    'tool_unload_speed': self.tool_unload_speed,
                    'tool_load_speed': self.tool_load_speed,
                    'buffer': self.buffer_name,
                    'lane_loaded': self.lane_loaded,
                    'tool_start': self.tool_start,
                    'tool_start_status': bool(self.tool_start_state),
                    'tool_end': self.tool_end,
                    'tool_end_status': bool(self.tool_end_state),
