...
class AFCTrigger:
    ...
    def __init__(self, config):
        ...
        self.gcode.register_mux_command("QUERY_BUFFER", "BUFFER", self.name, self.cmd_QUERY_BUFFER,
                                        desc=self.cmd_QUERY_BUFFER_help)
        self.gcode.register_mux_command("SET_BUFFER_VELOCITY", "BUFFER", self.name, self.cmd_SET_BUFFER_VELOCITY,
                                        desc=self.cmd_SET_BUFFER_VELOCITY_help)
        ...
        if self.turtleneck:
            ...
            self.gcode.register_mux_command("SET_ROTATION_FACTOR", "BUFFER", self.name, self.cmd_SET_ROTATION_FACTOR,
                                            desc=self.cmd_LANE_ROT_FACTOR_help)
            self.gcode.register_mux_command("SET_BUFFER_MULTIPLIER", "BUFFER", self.name,
        ...
        if state:
            if lane.status != 'unloading':
                if self.debug: self.logger.info(
                    "Buffer Triggered, Moving Lane {} forward {}mm".format(lane.name, self.buffer_distance))
    ...
    def set_multiplier(self, multiplier):
        ...
        if self.debug:
            ...
            self.logger.info(
                "New rotation distance after applying factor: {:.4f}".format(stepper.get_rotation_distance()[0]))
    ...
    def reset_multiplier(self):
        ...
        self.logger.info(
            "Rotation distance reset : {:.4f}".format(cur_stepper.extruder_stepper.stepper.get_rotation_distance()[0]))
        ...
        if self.turtleneck:
            ...
            if cur_stepper is not None and self.enable:
                ...
                if chg_multiplier == "HIGH" and chg_factor > 1:
                    ...
                    self.logger.info(
                        'multiplier_high: {} MUST be updated under buffer config for value to be saved'.format(
                            chg_factor))
                    ...
                    self.logger.info(
                        'multiplier_low: {} MUST be updated under buffer config for value to be saved'.format(
                            chg_factor))
                else:
