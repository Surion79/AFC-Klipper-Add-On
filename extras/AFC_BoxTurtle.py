...
try:
    ...
    raise error(
        "Error trying to import AFC_unit, please rerun install-afc.sh script in your AFC-Klipper-Add-On directory then restart klipper")


class AFC_BoxTurtle(afcUnit):
    ...
    def system_Test(self, cur_lane, delay, assignTcmd, enable_movement):
        ...
        cur_lane.do_enable(False)
        self.logger.info(
            '{lane_name} tool cmd: {tcmd:3} {msg}'.format(lane_name=cur_lane.name, tcmd=cur_lane.map, msg=msg))
    ...
    def calibrate_bowden(self, cur_lane, dis, tol):
        ...
        hub_pos, checkpoint, success = self.move_until_state(cur_lane, lambda: CUR_HUB.state, CUR_HUB.move_dis, tol,
                                                             cur_lane.short_move_dis, 0, cur_lane.dist_hub + 200,
                                                             "Moving to hub")
        ...
        if CUR_EXTRUDER.tool_start:
            ...
            while not cur_lane.get_toolhead_pre_sensor_state():
                ...
                if bow_pos >= fault_dis:
                    ...
                    msg += '\n SET_BOWDEN_LENGTH HUB={} LENGTH=+(distance the filament was short from the toolhead)'.format(
                        CUR_HUB.name)
                    return False, msg, bow_pos

            if CUR_EXTRUDER.tool_start != 'buffer':
                # is using ramming, only use first trigger of sensor
                bow_pos, checkpoint, success = self.calc_position(cur_lane,
                                                                  lambda: cur_lane.get_toolhead_pre_sensor_state(),
                                                                  bow_pos,
                                                                  cur_lane.short_move_dis, tol, 100,
                                                                  "retract from toolhead sensor")
    ...
    def calibrate_hub(self, cur_lane, tol):
        ...
        hub_pos, checkpoint, success = self.move_until_state(cur_lane, lambda: cur_lane.hub_obj.state,
                                                             cur_lane.hub_obj.move_dis,
                                                             tol, cur_lane.short_move_dis, hub_pos, hub_fault_dis,
                                                             checkpoint)
        ...
        if not success:
            ...
            msg += '\n SET_HUB_DIST LANE={} LENGTH=+(distance the filament was short from the hub)'.format(
                cur_lane.name)
            return False, msg, hub_pos
