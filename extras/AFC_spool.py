...
class AFCSpool:
    ...
    def handle_connect(self):
        ...
        self.gcode.register_command("RESET_AFC_MAPPING", self.cmd_RESET_AFC_MAPPING,
                                    desc=self.cmd_RESET_AFC_MAPPING_help)
    ...
    def set_spoolID(self, cur_lane, spool_id, save_vars=True):
        if self.AFC.spoolman is not None:
            if spool_id != '':
                try:
                    url = urlparse.urljoin(self.AFC.spoolman, "/api/v1/spool/{}".format(spool_id))
                    result = json.load(urlopen(url))
                    cur_lane.spool_id = spool_id
                    ...
                    if "multi_color_hexes" in result["filament"]:
                        cur_lane.color = "#{}".format(
                            self._get_filament_values(result["filament"], "multi_color_hexes").split(",")[0])
                    else:
                        cur_lane.color = "#{}".format(self._get_filament_values(result["filament"], "color_hex"))

                except Exception as e:
                    self.AFC.error.AFC_error(
                        "Error when trying to get Spoolman data for ID:{}, Error: {}".format(spool_id, e), False)
            else:
