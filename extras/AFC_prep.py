...
class afcPrep:
    def __init__(self, config):
        ...
        # Time to delay when moving extruders and spoolers during PREP routine
        ...
        # Set to True to disable remapping UNLOAD_FILAMENT macro to TOOL_UNLOAD macro
        self.dis_unload_macro = config.getboolean("disable_unload_filament_remapping", False)
    ...
    def _rename(self, base_name, rename_name, rename_macro, rename_help):
        ...
        if prev_cmd is not None:
            ...
            self.logger.debug(
                "{}Existing command {} not found in gcode_macros{}".format("<span class=warning--text>", base_name,
                                                                           "</span>", ))
    ...
    def _rename_macros(self):
        ...
        if not self.rename_occurred:
            ...
            self._rename(self.AFC.error.BASE_RESUME_NAME, self.AFC.error.AFC_RENAME_RESUME_NAME,
                         self.AFC.error.cmd_AFC_RESUME, self.AFC.error.cmd_AFC_RESUME_help)
            self._rename(self.AFC.error.BASE_PAUSE_NAME, self.AFC.error.AFC_RENAME_PAUSE_NAME,
                         self.AFC.error.cmd_AFC_PAUSE, self.AFC.error.cmd_AFC_RESUME_help)

            # Check to see if the user does not want to rename UNLOAD_FILAMENT macro
            if not self.dis_unload_macro:
                self._rename(self.AFC.BASE_UNLOAD_FILAMENT, self.AFC.RENAMED_UNLOAD_FILAMENT, self.AFC.cmd_TOOL_UNLOAD,
                             self.AFC.cmd_TOOL_UNLOAD_help)
    ...
    def PREP(self, gcmd):
        ...
        for extruder in self.AFC.tools.keys():
            printer_object = self.AFC.tools[extruder]
            self.AFC.tools[printer_object.name] = printer_object
            if 'system' in units and "extruders" in units["system"]:
                ...
                if printer_object.name in units["system"]["extruders"] and \
                        'lane_loaded' in units["system"]["extruders"][printer_object.name] and \
                        units["system"]["extruders"][printer_object.name]['lane_loaded']:
                    printer_object.lane_loaded = units["system"]["extruders"][printer_object.name]['lane_loaded']
                    self.AFC.current = printer_object.lane_loaded

        for lane in self.AFC.lanes.keys():
            cur_lane = self.AFC.lanes[lane]
            cur_lane.unit_obj = self.AFC.units[cur_lane.unit]
            if cur_lane.name not in cur_lane.unit_obj.lanes: cur_lane.unit_obj.lanes.append(
                cur_lane.name)  # add lanes to units list
            ...
            if cur_lane.unit in units:
                if cur_lane.name in units[cur_lane.unit]:
                    if 'spool_id' in units[cur_lane.unit][cur_lane.name]: cur_lane.spool_id = \
                        units[cur_lane.unit][cur_lane.name]['spool_id']
                    if self.AFC.spoolman != None and cur_lane.spool_id:
                        self.AFC.spool.set_spoolID(cur_lane, cur_lane.spool_id, save_vars=False)
                    else:
                        if 'material' in units[cur_lane.unit][cur_lane.name]: cur_lane.material = \
                            units[cur_lane.unit][cur_lane.name]['material']
                        if 'color' in units[cur_lane.unit][cur_lane.name]: cur_lane.color = \
                            units[cur_lane.unit][cur_lane.name]['color']
                        if 'weight' in units[cur_lane.unit][cur_lane.name]: cur_lane.weight = \
                            units[cur_lane.unit][cur_lane.name]['weight']
                    if 'runout_lane' in units[cur_lane.unit][cur_lane.name]: cur_lane.runout_lane = \
                        units[cur_lane.unit][cur_lane.name]['runout_lane']
                    if cur_lane.runout_lane == '': cur_lane.runout_lane = 'NONE'
                    if 'map' in units[cur_lane.unit][cur_lane.name]: cur_lane.map = units[cur_lane.unit][cur_lane.name][
                        'map']
                    ...
                    if 'hub_loaded' in units[cur_lane.unit][cur_lane.name]: lane.loaded_to_hub = \
                        units[cur_lane.unit][cur_lane.name]['hub_loaded']
                    # Check for loaded_to_hub as this is how its being saved version > 1030
                    if 'loaded_to_hub' in units[cur_lane.unit][cur_lane.name]: cur_lane.loaded_to_hub = \
                        units[cur_lane.unit][cur_lane.name]['loaded_to_hub']
                    if 'tool_loaded' in units[cur_lane.unit][cur_lane.name]: cur_lane.tool_loaded = \
                        units[cur_lane.unit][cur_lane.name]['tool_loaded']
        ...
        for unit in self.AFC.units.keys():
            ...
            if len(lanes_for_first_hub) != 0:
                self.logger.raw(
                    "<span class=warning--text>No hub defined in lanes or unit for {unit}. Defaulting to {hub}</span>".format(
