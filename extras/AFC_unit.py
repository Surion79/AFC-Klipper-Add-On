...
try:
    ...
    raise error(
        "Error trying to import AFC_respond, please rerun install-afc.sh script in your AFC-Klipper-Add-On directory then restart klipper")
...
class afcUnit:
    def __init__(self, config):
        ...
        # Hub name(AFC_hub) that belongs to this unit, can be overridden in AFC_stepper section
        self.hub = config.get("hub", None)

        # Extruder name(AFC_extruder) that belongs to this unit, can be overridden in AFC_stepper section
        self.extruder = config.get("extruder", None)

        # Buffer name(AFC_buffer) that belongs to this unit, can be overridden in AFC_stepper section
        self.buffer_name = config.get('buffer', None)

        self.led_name = config.get('led_name', self.AFC.led_name)

        # LED color to set when faults occur in lane (R,G,B,W) 0 = off, 1 = full brightness. Setting value here overrides values set in AFC.cfg file
        self.led_fault = config.get('led_fault', self.AFC.led_fault)

        # LED color to set when lane is ready (R,G,B,W) 0 = off, 1 = full brightness. Setting value here overrides values set in AFC.cfg file
        self.led_ready = config.get('led_ready', self.AFC.led_ready)

        # LED color to set when lane not ready (R,G,B,W) 0 = off, 1 = full brightness. Setting value here overrides values set in AFC.cfg file
        self.led_not_ready = config.get('led_not_ready', self.AFC.led_not_ready)

        # LED color to set when lane is loading (R,G,B,W) 0 = off, 1 = full brightness. Setting value here overrides values set in AFC.cfg file
        self.led_loading = config.get('led_loading', self.AFC.led_loading)

        # LED color to set when lane is loaded (R,G,B,W) 0 = off, 1 = full brightness. Setting value here overrides values set in AFC.cfg file
        self.led_prep_loaded = config.get('led_loading', self.AFC.led_loading)

        # LED color to set when lane is unloading (R,G,B,W) 0 = off, 1 = full brightness. Setting value here overrides values set in AFC.cfg file
        self.led_unloading = config.get('led_unloading', self.AFC.led_unloading)

        # LED color to set when lane is loaded into tool (R,G,B,W) 0 = off, 1 = full brightness. Setting value here overrides values set in AFC.cfg file
        self.led_tool_loaded = config.get('led_tool_loaded', self.AFC.led_tool_loaded)

        # Speed in mm/s to move filament when doing long moves. Setting value here overrides values set in AFC.cfg file
        self.long_moves_speed = config.getfloat("long_moves_speed", self.AFC.long_moves_speed)

        # Acceleration in mm/s squared when doing long moves. Setting value here overrides values set in AFC.cfg file
        self.long_moves_accel = config.getfloat("long_moves_accel", self.AFC.long_moves_accel)

        # Speed in mm/s to move filament when doing short moves. Setting value here overrides values set in AFC.cfg file
        self.short_moves_speed = config.getfloat("short_moves_speed", self.AFC.short_moves_speed)

        # Acceleration in mm/s squared when doing short moves. Setting value here overrides values set in AFC.cfg file
        self.short_moves_accel = config.getfloat("short_moves_accel", self.AFC.short_moves_accel)

        # Move distance in mm for failsafe moves. Setting value here overrides values set in AFC.cfg file
        self.short_move_dis = config.getfloat("short_move_dis", self.AFC.short_move_dis)

        # Maximum distance to move filament. AFC breaks filament moves over this number into multiple moves. Useful to lower this number if running into timer too close errors when doing long filament moves. Setting value here overrides values set in AFC.cfg file
        self.max_move_dis = config.getfloat("max_move_dis", self.AFC.max_move_dis)

        # Time to wait between breaking n20 motors(nSleep/FWD/RWD all 1) and then releasing the break to allow coasting. Setting value here overrides values set in AFC.cfg file
    ...
    def handle_connect(self):
        ...
        self.gcode.register_mux_command("UNIT_CALIBRATION", "UNIT", self.name, self.cmd_UNIT_CALIBRATION,
                                        desc=self.cmd_UNIT_CALIBRATION_help)
        self.gcode.register_mux_command("UNIT_LANE_CALIBRATION", "UNIT", self.name, self.cmd_UNIT_LANE_CALIBRATION,
                                        desc=self.cmd_UNIT_LANE_CALIBRATION_help)
        self.gcode.register_mux_command("UNIT_BOW_CALIBRATION", "UNIT", self.name, self.cmd_UNIT_BOW_CALIBRATION,
                                        desc=self.cmd_UNIT_BOW_CALIBRATION_help)
    ...
    def get_status(self):
        ...
        for lane in self.lanes.values():
            ...
            if lane.extruder_name is not None and lane.extruder_name not in response["extruders"]: response[
                "extruders"].append(lane.extruder_name)
            if lane.buffer_name is not None and lane.buffer_name not in response["buffers"]: response["buffers"].append(
                lane.buffer_name)
