...
class AFC_HTLF(AFC_BoxTurtle):
    ...
    def __init__(self, config):
        ...
        # CAM lobe angle thats currently installed. 30,45,60 (recommend using 60)
        self.cam_angle = config.getint("cam_angle")
        self.home_pin = config.get("home_pin")  # Pin for homing sensor

        # Max angle to move lobes, this is when lobe 1 is fully engauged with its lane
        self.MAX_ANGLE_MOVEMENT = config.getint("MAX_ANGLE_MOVEMENT", 215)
        ...
        if self.cam_angle not in self.VALID_CAM_ANGLES:
            raise error("{} is not a valid cam angle, please choose from the following {}".format(self.cam_angle,
                                                                                                  self.VALID_CAM_ANGLES))
    ...
    def return_to_home(self, prep=False):
        ...
        if self.current_selected_lane is not None and not self.home_state and not prep:
            self.selector_stepper_obj.move(self.calculate_lobe_movement(self.current_selected_lane.index) * -1, 20, 20,
                                           False)
